...
/**
 * Perform a hybrid RAG retrieval across the user's notes and artifacts,
 * returned as one ranked list. Note hits collapse to one per underlying
 * article (duplicate saves of the same content) and carry ``note``/
 * ``article_id``; artifact hits collapse to one per artifact and carry
 * ``artifact``/``artifact_id`` with null article/note fields.
 */
